  ...
  return (
    <>
      ...
      <div className="w-[720px] mx-auto flex justify-between mt-31">
        ...
        <button className="border rounded-[14px] py-2.5 w-[38%] cursor-pointer" onClick={() => addTask(inputValue)}>Create task</button>
      </div>
            ...
            {tasks.map(task => (
              <div className="flex justify-between">
                  ...
                  <div className="flex justify-between w-[28%]">
                    <button onClick={() => deleteTask(task.id)} className="cursor-pointer"><img src={deleteIcon} alt="Delete" /></button>
                    <button onClick={() => completeTask(task.id)} className="cursor-pointer w-[75px] h-[33px] border rounded-[11px] flex justify-center items-center">{task.completed && <img className="h-[25px]" src={check} alt="asdds" />}</button>
                    <button onClick={() => undoneTask(task.id)} className="cursor-pointer w-[75px] h-[33px] border rounded-[11px]  flex justify-center items-center">{task.undone && <img className="h-[25px]" src={close} alt="asdds" />}</button>
                  </div>
              </div>
            ))}
          </div>
